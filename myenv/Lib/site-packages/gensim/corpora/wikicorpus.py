...
def remove_template(s):
    """Remove templates wikimedia markup.

    Parameters
    ----------
    s : str
        String containing markup templates.

    Returns
    -------
    str
        Сopy of `s` with all the `wikimedia markup templates <http://meta.wikimedia.org/wiki/Help:Template>`_ removed.

    Notes
    -----
    Since templates can be nested, it is difficult remove them using regular expressions.

    """
    # Find the start and end position of each templates by finding the opening
    # '{{' and closing '}}'
    n_open, n_close = 0, 0
    starts, ends = [], [-1]
    in_template = False
