...
def schema(
    ...
    *,
    ...
    :param ref_prefix: the JSON Pointer prefix for schema references with ``$ref``, if None, will be set to the
      ...
      prefix.
    :param ref_template: Use a ``string.format()`` templates for ``$ref`` instead of a prefix. This can be useful
...
def model_schema(
    ...
    :param ref_prefix: the JSON Pointer prefix for schema references with ``$ref``, if None, will be set to the
      ...
      top-level key ``definitions``, so you can extract them from there. But all the references will have the set
      prefix.
    :param ref_template: Use a ``string.format()`` templates for ``$ref`` instead of a prefix. This can be useful for
