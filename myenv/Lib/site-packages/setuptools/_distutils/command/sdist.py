...
class sdist(Command):
    ...
    user_options = [
        ('templates=', 't', "name of manifest templates file [default: MANIFEST.in]"),
    ...
    def initialize_options(self):
        # 'templates' and 'manifest' are, respectively, the names of
        # the manifest templates and manifest file.
        self.template = None
        self.manifest = None
    ...
    def run(self):
        ...
        # (process the manifest templates, read an existing manifest,
    ...
    def get_file_list(self):
        ...
        reading the manifest templates (and writing the manifest), or just
        reading the manifest, or just using the default file set -- it all
        depends on the user's options.
        """
        # new behavior when using a templates:
        ...
        if not template_exists:
            self.warn(
                ("manifest templates '%s' does not exist " + "(using default file list)")
                % self.template
            )
    ...
    def read_template(self):
        """Read and parse manifest templates file named by self.templates.

        (usually "MANIFEST.in") The parsing and processing is done by
        'self.filelist', which updates itself accordingly.
        """
        log.info("reading manifest templates '%s'", self.template)
        template = TextFile(
            self.template,
            strip_comments=1,
            skip_blanks=1,
